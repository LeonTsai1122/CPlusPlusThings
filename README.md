...
本專案包含了一個基礎的 C 語言專案架構，適合初學者學習如何使用 `gcc` 編譯程式及撰寫 `Makefile`。
...
1. **編譯程式**:
   在終端機中執行 `make` 指令，將會自動編譯原始碼檔案並產生名為 `main` 的執行檔。
   ```bash
   make
   ```
...
2. **執行程式**:
   ...
   ./main
   ```
   預期會看到封包解析的測試結果輸出，驗證 Header、Command、Length、Payload 與 Checksum 是否正確解析。

3. **清理專案**:
   執行 `make clean` 可以清除編譯出來的執行檔與相關產物。
   ```bash
   make clean
   ```
